...
async function browserJson(url, options = {}, settings = {}) {
  ...
  const response = await evaluateFetch(session.cdp, url, options);

  // 注意：fetch 错误（网络/API 断连）不关闭共享会话——并发批处理中一个请求失败会
  // 连带杀死同批其他在途请求（ws 关闭）。会话死活由下次调用前的 ping 自动检测重连。
  try {
...
function scheduleDisconnect() {
  ...
  // 空闲 90 秒后断开浏览器会话（并发批处理扫描中请求间隔可能较长，短超时会中途断连）
  disconnectTimer = setTimeout(disconnectBrowserSession, 90000);
