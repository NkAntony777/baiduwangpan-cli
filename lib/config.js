...
function findPCS() {
  // 1. Look in package root (postinstall target)
  const pkgRoot = path.join(__dirname, "..");
  for (const name of ["BaiduPCS-Go.exe", "BaiduPCS-Go"]) {
    const p = path.join(pkgRoot, name);
    if (fs.existsSync(p)) return p;
  }
  // 2. Look in global node_modules
  const nmGlobal = path.join(__dirname, "..", "..", "BaiduPCS-Go.exe");
  if (fs.existsSync(nmGlobal)) return nmGlobal;
  // 3. Fallback to PATH
  return "BaiduPCS-Go";
}
...
module.exports = { get, set, update, setAuth, load, save, isLoggedIn, CONFIG_DIR, CONFIG_FILE };
