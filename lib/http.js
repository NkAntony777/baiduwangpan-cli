...
function curlGet(url, extraHeaders = {}) {
  ...
  const headers = {
    Cookie: `BDUSS=${cfg.bduss}; STOKEN=${cfg.stoken}`,
...
async function getBdstoken() {
  ...
  if (!token) {
    ...
    throw new Error(
      `Cannot get bdstoken. Check BDUSS/STOKEN.\n` +
      `  BDUSS: ${cfg.bduss ? "set" : "MISSING"}  STOKEN: ${cfg.stoken ? "set" : "MISSING"}\n` +
      `  Run: bdp login --bduss <value> --stoken <value>`
    );
  }
  return token;
}
...
module.exports = { curlGet, curlPost, curlRange, curlJson, getBdstoken, APP_ID, PAN_BASE };
