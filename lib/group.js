...
function describeErrno2131(gidS, msgIdS) {
  return (
    `shareinfo rejected: msg_id ${msgIdS} 不属于群 ${gidS} 的任何分享 (errno=2131)。` +
...
async function shareInfoRaw(gidS, fsIdS, fromUkS, msgIdS, page, pageSize) {
  ...
  if (isSelfEcho(data, fsIdS)) {
    const e = new ShareApiError(-3, `shareinfo throttled (self-echo) for fs_id ${fsIdS}`);
    e.throttled = true;
    throw e;
  }
  return data;
...
async function shareInfoRawWithRetry(gidS, fsIdS, fromUkS, msgIdS, page, pageSize, budget, throttleState) {
  if (budget) {
    if (budget.remaining <= 0) throw new ShareApiError(-3, "shareinfo request budget exhausted");
    budget.remaining--;
  }
  let attempts = 0;
  for (;;) {
...
async function resolveShareSource(gidS, fsIdS, options = {}, { cache }) {
  ...
  if (!fromUkS || !msgIdS) {
    ...
    if (!match) {
      throw new Error(
        `Cannot determine fromUk/msgId for fsId ${fsIdS} (not a top-level share). ` +
        `Pass --from-uk and --msg-id explicitly (use bdp gsearch/gsearch --json to get them).`
      );
    }
    fromUkS = match.fromUk;
    msgIdS = match.msgId;
...
async function getShareDlink(gid, fsId, options = {}) {
  ...
  const data = await http.webJson(url, { method: "POST", body });
  if (data.errno !== 0) {
    throw new Error(`API error: errno=${data.errno} (msg_id ${msgId} 不属于群 ${gidS} 或分享已失效)`);
  }
  const item = (data.list || [])[0];
  if (!item) throw new Error(`No file info returned for fsId ${fsIdS}`);
  ...
  if (!result.dlink) {
    throw new Error(`No dlink returned for fsId ${fsIdS}（分享可能已失效或禁止下载）`);
  }
  return result;
}
...
async function downloadFile(gid, fsId, options = {}) {
  ...
  if (info.size && size !== info.size) {
    throw new Error(
      `Download size mismatch: got ${size} bytes, expected ${info.size} bytes. ` +
      `dlink 可能已过期（有效期 8h），请重试。`
    );
  }
...
async function getShareZipDlink(gid, fsIds, options = {}) {
  ...
  if (fsIdList.length < 2) throw new Error(`getShareZipDlink needs >= 2 fs_ids`);
  ...
  const data = await http.webJson(url, { method: "POST", body });
  if (data.errno !== 0) {
    throw new Error(`API error: errno=${data.errno} (zip batch)`);
  }
  if (!data.dlink) {
    throw new Error(`No zip dlink returned for ${fsIdList.length} files`);
  }
...
async function downloadFiles(gid, fsIds, options = {}) {
  ...
  try { size = fs.statSync(outPath).size; } catch {}
  if (size === 0) {
    throw new Error(`Zip download failed (0 bytes). dlink 可能已过期或包太大（error_code 31090）。`);
  }
...
async function searchFiles(gid, keyword, options = {}) {
  ...
  const rawKw = String(keyword).toLowerCase();
  const words = rawKw.split(/\s+/).filter(Boolean);
  const matchMode = options.exact ? "exact" : (options.anyWord ? "any" : "all");
  const matchesName = (name) => {
    const n = String(name || "").toLowerCase();
    if (matchMode === "exact") {
      // 精确匹配按文件名（去掉路径前缀），顶层分享 name 形如 "/xxx"，子目录文件 name 无前缀
      const base = n.includes("/") ? n.slice(n.lastIndexOf("/") + 1) : n;
      return base === rawKw;
    }
    if (words.length <= 1) return n.includes(rawKw);
    if (matchMode === "any") return words.some((w) => n.includes(w));
